...
class Register_two : AppCompatActivity(), View.OnClickListener, TextWatcher {
   ...
   * */
    ...
    private fun regist() {
        ...
        if (TextUtils.isEmpty(uname) && !TextUtils.isEmpty(pass) && !TextUtils.isEmpty(incodes)) {
            ...
            if (pass.length < 6) {
                ...
                Log.d(TAG, "regist: 添加")
                userDao.insertUser(User(phone = phonenumber!!,
                    password = pass,
                    identity =  identity,
                    identityNumber = incodes,
                    name = uname))
    ...
    object : Handler(Looper.getMainLooper()) {     //此处的object 要加，否则无法重写 handlerMessage
        override fun handleMessage(msg: Message) {
            ...
            if (opreation == "ResponseInsert"){
                ...
                Toast.makeText(this@Register_two,"未知错误，请联系开发者获得更多支持", Toast.LENGTH_SHORT).show()}
